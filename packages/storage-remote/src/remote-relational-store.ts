...
export class RemoteRelationalStore implements IRelationalStore {
  ...
  async beginTransaction(sid: string): Promise<void> { await this._w(); await this.e('BEGIN'); await this.e('SAVEPOINT sp_'+this.esc(sid)); await this.q('INSERT INTO analysis_state VALUES ($1,$2,$3,$4) ON CONFLICT (session_id, checkpoint_name) DO UPDATE SET stage = EXCLUDED.stage', [sid,'started',null,null]); }
  async commitTransaction(sid: string): Promise<void> { await this._w(); await this.e('RELEASE SAVEPOINT sp_'+this.esc(sid)); await this.e('COMMIT'); }
