...
export class RemoteRelationalStore implements IRelationalStore {
  ...
  async writeDetections(d: DetectionResult[], signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); for (const x of d) for (let i = 0; i < x.scores.length; i++) await this.q('INSERT INTO metrics VALUES ($1,$2,$3) ON CONFLICT (ts, metric_name) DO UPDATE SET value = EXCLUDED.value', [x.timestamp, x.scores[i]!, 'm' + i]); }
  async saveCPT(gid: string, n: string, cpt: ConditionalProbabilityTable, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); for (const [s, p] of Object.entries(cpt.entries)) await this.q('INSERT INTO cpt VALUES ($1,$2,$3,$4) ON CONFLICT (graph_id, node, parent_state) DO UPDATE SET prob = EXCLUDED.prob', [gid, n, s, p]); }
  async loadCPT(gid: string, n: string, signal?: AbortSignal): Promise<ConditionalProbabilityTable|null> { await this._w(); signal?.throwIfAborted();  const r = await this.q('SELECT parent_state, prob FROM cpt WHERE graph_id = $1 AND node = $2 ORDER BY parent_state', [gid, n]); const x = r.rows as Array<{parent_state:string;prob:number}>; if (!x.length) return null; const e: Record<string,number> = {}; for (const v of x) e[v.parent_state] = v.prob; return { node: n, parents: [], entries: e }; }
  async saveRegressionModel(gid: string, n: string, m: RegressionParams, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); signal?.throwIfAborted();  await this.q('INSERT INTO regression_models VALUES ($1,$2,$3,$4,$5) ON CONFLICT (graph_id, node) DO UPDATE SET coefficients = EXCLUDED.coefficients, intercept = EXCLUDED.intercept, residual_std = EXCLUDED.residual_std', [gid, n, JSON.stringify(m.coefficients), m.intercept, m.residualStdDev]); }
  async loadRegressionModel(gid: string, n: string, signal?: AbortSignal): Promise<RegressionParams|null> { await this._w(); signal?.throwIfAborted();  const r = await this.q('SELECT coefficients, intercept, residual_std FROM regression_models WHERE graph_id = $1 AND node = $2', [gid, n]); const x = r.rows as any[]; return x.length>0 ? { coefficients: JSON.parse(x[0].coefficients), intercept: x[0].intercept, residualStdDev: x[0].residual_std } : null; }
  async saveRCAResult(cid: string, r: RCAResult, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted(); await this._w(); await this.q('INSERT INTO rca_results VALUES ($1,$2,$3,$4) ON CONFLICT (case_id) DO UPDATE SET result_json = EXCLUDED.result_json', [cid, JSON.stringify(r.toJSON()), Date.now(), r.rootCauses[0]?.name ?? null]); }
  ...
  async beginTransaction(sid: string, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); signal?.throwIfAborted();  await this.e('BEGIN'); await this.e('SAVEPOINT sp_'+this.esc(sid)); await this.q('INSERT INTO analysis_state VALUES ($1,$2,$3,$4) ON CONFLICT (session_id, checkpoint_name) DO UPDATE SET stage = EXCLUDED.stage', [sid,'started',null,null]); }
  async commitTransaction(sid: string, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); signal?.throwIfAborted();  await this.e('RELEASE SAVEPOINT sp_'+this.esc(sid)); await this.e('COMMIT'); }
  async rollbackToCheckpoint(_sid: string, cp: string, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); signal?.throwIfAborted();  await this.e('ROLLBACK TO SAVEPOINT sp_'+this.esc(cp)); }
  async setCheckpoint(sid: string, name: string, signal?: AbortSignal): Promise<void> { signal?.throwIfAborted();  await this._w(); signal?.throwIfAborted();  await this.e('SAVEPOINT sp_'+this.esc(name)); await this.q('INSERT INTO analysis_state VALUES ($1,$2,$3,$4) ON CONFLICT (session_id, checkpoint_name) DO UPDATE SET stage = EXCLUDED.stage', [sid,'checkpoint',name,null]); }
