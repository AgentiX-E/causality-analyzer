...
describe('GOLEM Algorithm', () => {
  it('discovers DAG from linear chain data', { timeout: 15000 }, () => {
    ...
    const result = golemAlgorithm(data, nodeNames, { maxIter: 4000, lambda1: 0.002 });
  ...
  it('produces valid W matrix dimensions', () => {
    ...
    const result = golemAlgorithm(data, nodeNames, { maxIter: 2000 });
  ...
  it('respects lambda1 regularization', () => {
    ...
    const r1 = golemAlgorithm(data, nodeNames, { lambda1: 0.5, maxIter: 800 });
    const r2 = golemAlgorithm(data, nodeNames, { lambda1: 0.001, maxIter: 800 });
  ...
  it('applies domain knowledge', () => {
    ...
    const result = golemAlgorithm(data, nodeNames, { maxIter: 1000, lambda1: 0.003 }, {
      forbids: [['Z', 'X']],
    });
    expect(result.graph.nodeCount).toBe(3);
  });
  ...
  it('GOLEM scales to 4-variable graph', () => {
    ...
    const result = golemAlgorithm(data, nodeNames, { maxIter: 2500, lambda1: 0.003 });
